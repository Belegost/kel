window.addEventListener('load', function() {
    ...
    $('#code').mask('000000');
    changeStep('.' + initDepositStep);

    let fillProfileFormName = 'fill_profile',
        $form = $("form[name=" + fillProfileFormName + "]");
    ...
    $form.submit(event => {
        ...
        $.ajaxSetup({
            url: '/account/fill-account-data',
            global: false,
            type: "POST",
            processData: false,
            contentType: false,
            success: (response) => {
                showFromError(fillProfileFormName, response);
                if (response.success === true) {
                    changeStep('.' + response.next_step)
                }
            },
            error: (response) => {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status == 'success') {
                    ...
                    $('.deposit-addresses').html(response.data.body);

                    changeStep('.deposit-addresses');
                    // $('body').removeClass('deposit').addClass('deposit-cf');
